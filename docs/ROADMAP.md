...
## A 组 · 章节创作区（正文生成 / 去 AI 味 / 审校 / UI 隔离）—— 🔴 重灾区

### A-1 `G5` 每章 AI 生成无 UI 隔离，面板"串台" 🔴🔴
...
### A-4 `G4` 去 AI 味后字数严重缩水（2000+ 字 → 845 字）🔴
...
### A-7 `G7` 质量审校 / 去 AI 味 / 追读力 报告收起再展开就消失 🔴
...
### A-8 `G8` 审校报告支持 AI 一键修改（参照「去 AI 味」）🟡
...
## B 组 · 大纲生成流程（卷纲 / 章纲 批量化与正确性）

### B-1 `B3` 建议卷数不生效（设 20 卷只生成 2 卷）🔴
...
### C-2 `F2` AI 补充词条标签（开关 + ⚠️ 说明）🟡
- **现象**：希望 AI 能补充词条标签。
- **方案**：加「AI 补充词条」开关 + ⚠️ 说明（会消耗 token）。依赖 C-1 的词条结构。
- **优先级**：🟡 中（依赖 C-1）。

### C-3 `G12` 物品栏：提取不准 + 表现差，需重新规划 🔴
- **现象**：物品栏提取不够准，展示也不行。
...
### C-5 `G13` 剧情大事（故事进程年表）UI 表现需重新规划 🟡
- **现象**：剧情大事的展示表现差。
- **方案**：重做时间线 UI（按故事进程排布、重要度标识、关联章节跳转），提升可读性。提取逻辑已是 AI，本条主要是表现层。
- **优先级**：🟡 中。

### C-6 `G10` 重要地点：加"AI 从正文拆解补全"能力 🟡
...
### E-1 `B1` 一级标签切换丢失二级 AI 生成状态（设定库 + 创作区普遍）🔴
...
> **建议处理批次**（按依赖与收益）：
> 1. **统一隔离机制**：A-1(G5) + A-7(G7) + E-1(B1) 同根，先建一套"AI 生成态按上下文键隔离 + 持久化"，一次解决三处。
> 2. **去 AI 味闭环**：A-2(G1 确认) + A-3(G2 可关闭) + A-4(G4 字数约束) + A-8(G8 审校 AI 改)，一组体验闭环。
> 3. **大纲生成流程专项**：B-1~B-6 一起改（卷数注入 + 接续不重复 + 空卷空章入口 + 批量文案）。
> 4. **设定库提取统一**：C-1(词条 AI 拆分) 打底，C-6(地点提取)、E-3(信仰采纳) 复用同一"正文 / 内容 → 结构化设定"AI 流程；C-3 物品栏 / C-4 状态卡 / C-5 年表 各自重做表现。
> 5. **角色重构 D-1**：工程量大，单独立项、先对齐设计。
> 6. 零散打磨：A-5(正文字数)、A-6(空行)、C-7(道具文案)、E-2(标签宽度)、E-4(文风状态入口)。
