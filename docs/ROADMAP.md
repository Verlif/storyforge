...
- ⬜ **买辣椒：正文粘贴内容切换页面后格式丢失** — 根因确认：`useAutoSave` 的 debounce（1.5s）在组件卸载时只做 `clearTimeout` 不做 flush，用户编辑后 1.5s 内切走侧栏模块会导致保存被取消、数据丢失。修复方案：`useAutoSave` 的 cleanup 里在 clearTimeout 后立即执行一次 `saveFn(latestData)`，确保 unmount 前数据落盘。文件：`src/hooks/useAutoSave.ts`
