...
## 2026-06-09 · 🏛️ 大架构重构上线(v3.0.0)

> 完整版本日志见根目录 `CHANGELOG.md` 的 v3.0.0 条目。这里记要点。

### 三注册表地基(本次核心架构变化)

把全项目的"读 / 写 / 表生命周期"收口到**三个单一事实源注册表**,根治"散落手写 → 反复冒同类 bug"的结构性病根:

- **① `PROJECT_TABLES` + 派生生命周期 API**:导出/导入/删除/迁移统一派生,**加表只改一处**;取代此前各处手列表清单(漏表反复发生)。
- **② `FIELD_REGISTRY` + `AdoptionSchema` + `adopt()`**:AI 写回统一走采纳层(字段映射/去重/外键/类型校验);取代散落 `db.x.add/update`。
- **③ `CONTEXT_SOURCES` + `assembleContext()`**:AI 读上下文统一装配(18 源、按预算 L3→L2→L1 裁剪);取代面板手挑 builder、多世界串台。
- 配套:启动期注册表校验 + **CI 架构 lint**(自动拦截绕过注册表的手写代码)+ **AI 说明书自动生成**(防文档漂移)+ 项目宪法 `CLAUDE.md`。

### 数据安全 8 个 P0 根治(可能损坏用户手稿的高危)

`deleteGroup`/`migrate` 事务作用域不全、`ensureSchema` 删库风险、`BUG-EXPORT-WG` 多世界导出键值错位、`importProjectJSON` 非事务+外键不校验、`deleteProject` 漏间接归属表、`deleteNode` 绕过章节级联、`migrate` 漏给大纲盖章——全部修复并各配反例测试。

### 多世界 + 内容完整性(Phase 2 P1 一批)

真实与幻想多世界化、章节正文接世界规则、AIFieldCard 带当前值生成、分块导入按目标世界盖章、批量正文按章读世界上下文、角色删除/合并引用清理、状态卡选择性召回,等。

### 工程化(Phase 3)

测试 0→87 个(含 18 反例)、CI 6 道关、PAT 默认 session-only、主包 gzip -30%、文档体系、i18n 框架预留。

### 社区反馈

- **FB-1**:工作流第2步「世界起源」不读第1步、上下文全空 → 双根因修复(闭包累加器 + 走 assembleContext),真实 AI 抓包验证。
- **FB-3(部分)**:世界起源生成补读「一句话故事」。

> 验证:tsc=0 / 87 测试 / 架构 lint 0 违规 / build OK。其余反馈(FB-2/4/5/6/7/8)与完善性专项(HEALTH-1~6)见 `docs/ROADMAP.md`。
